...
angular.module('newsApp')
  .controller('GradesListCtrl', ['$scope', '$rootScope', '$http', '$location', 'paginator', 'Grades',
    function($scope, $rootScope, $http, $location, paginator, Grades) {
      ...
      $scope.saveOrder = function() {
        for (var i in $scope.data.records) {
          ...
          $scope.order.isDirty = true;
          Grades.update($scope.data.records[i]._id, data).then(function onSuccess(response) {
    ...
    function($scope, $rootScope, Grades, $log, $stateParams,
      ...
      Grades.findOne($stateParams.id).then(function(response) {
        ...
        if ($scope.data.model.media.length > 0) {
          ...
          //
          // mediaID = $scope.grade.media[0];
          //
          // Media.findOne(mediaID)
          //   .then(function onSuccess(response) {
          //     $scope.grade.picture = response.data.result.uri;
          //   }, function onError(response) {
       ...
       */
      $scope.deleteImage = function(grade) {
        deleteImage();
      };
      ...
      $scope.update = function() {
        ...
        function saveGrade(mediaIds) {
          ...
          Grades.update($scope.data.model._id, data).then(function onSuccess(response) {
            progressEnd();
            // notify the listener when the grade is added
...
angular.module('newsApp')
  .controller('GradesDetailsCtrl', ['$scope', '$stateParams',
    'Grades', 'Media', '$log',
    function($scope, $stateParams, Grades, Media, $log) {
